...
def get_vnstat_totals(interface: str) -> tuple[int, int]:
    ...
    raise SystemExit(f"找不到网卡 {interface} 的 vnstat total 数据")
...
def main() -> None:
    ...
    rx, tx = get_vnstat_totals(cfg["interface"])
    ...
    if st["cycle_start_ts"] != cstart:
        prev_rx = max(0, rx - st["start_rx"])
        prev_tx = max(0, tx - st["start_tx"])
